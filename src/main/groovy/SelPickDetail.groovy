  ...
  public void main() {
    ...
    def callback = {
      Map <String, String> out ->
        ...
        int minBestBeforeDate = 0;
        minBestBeforeDate = getMinBestBeforeDate(out);
    ...
    if (ttyp == 31) {
      ...
      if(query.read(OOLINE)){
        ...
        if(query.read(OCUSIT)){
          LocalDate date = LocalDate.parse(OOLINE.getInt("OBDWDZ").toString(), DateTimeFormatter.ofPattern("yyyyMMdd"));
          date = date.plusDays(OCUSIT.getInt("ORADCU"));
          bestDate = date.format(DateTimeFormatter.ofPattern("YYYYMMdd")).toInteger();
      ...
      if(query.read(MGLINE)){
        ...
        if(query.read(MITBAL)){
          LocalDate date = LocalDate.parse(MGLINE.getInt("MRTRDT").toString(), DateTimeFormatter.ofPattern("yyyyMMdd"));
