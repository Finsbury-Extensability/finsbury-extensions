...
public class LstLvl4Codes extends ExtendM3Transaction {
  ...
  public LstLvl4Codes(MIAPI mi, DatabaseAPI database, ProgramAPI program) {
    this.mi = mi;
  ...
  public void main() {
    ...
    String level3Code = mi.inData.ZLV3
    ...
    containerCUGEX1.set("F1PK04", level3Code)
    queryCUGEX1.readAll(containerCUGEX1, 6, handler)
  }
  ...
  Closure<?> handler = {DBContainer result ->
    if (!result.getString("F1PK04").isBlank() && !result.getString("F1PK05").isBlank()) {
      ...
      mi.outData.put("ZLV4", result.getString("F1PK05"))
